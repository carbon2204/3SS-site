function changeLanguage(newLanguage) {
    ...
    // Обновляем заголовок страницы на основе текущего navItemId
    const currentItem = belhardSiteData.form.data.navigation.find(
        item => String(item.id) === String(belhardSiteData.currentNavItemId)
    );
    document.title = currentItem && currentItem.title && currentItem.title[newLanguage] ?
        currentItem.title[newLanguage] :
        belhardSiteData.title[newLanguage];
  
    // Удаляем существующие элементы
    const existingNavBar = document.getElementById('navbar');
    const existingSlider = document.getElementById('sliderContentContainer');
    const existingContactForm = document.querySelector('.contact-wrapper');
    const existingFooter = document.getElementById('footer');
    const existingImageBlock = document.querySelector('.image-blocks-container');
    const existingCatalogContent = document.querySelector('.main-content');
    const existingFormContent = document.querySelector('.form-container');
    ...
    if (existingFormContent) {
        existingFormContent.remove();
        createFormAndContent(
            belhardSiteData.form,
            belhardSiteData.navBar,
            belhardSiteData.currentLanguage,
            belhardSiteData.currentNavItemId,
            belhardSiteData.form.formSource.navigationSource,
            window.defaultStyles.aboutStyles
        );
    }
    ...
    if (existingCatalogContent) {
        existingCatalogContent.remove();
        const currentMenuItem = belhardSiteData.navBar[3];
        createCatalogPage(currentMenuItem, belhardSiteData.catalogData, newLanguage, window.defaultStyles.catalogStyles);
    }
}
...
function createLanguageSelect(languages) {
    ...
    selectBox.addEventListener("click", function (e) {
        ...
        if (isOpen) {
            ...
            dropdown.style.display = 'block'; // Сначала показываем
            setTimeout(function () {
